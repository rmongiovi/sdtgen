...
/* Data used by the scanner and parser generator */

   int		  display;		/* Selected display options */
   int		  debug;		/* Debug flag bits */
   bool		  process;		/* True if scanner and parser tables should be generated */
   int		  options;		/* Option flags set by input grammar */
   dynarray	  semstack;		/* Semantic stack for parse tree */
   tokenvalue	  tokenval;		/* Values for next defined token */
   symbolentry	**termtable;		/* Map of terminal token values to symbols */
   int		  termcount;		/* Number of defined terminals */
   symbolentry	**nontermtable;		/* Map of nonterminal token values to symbols */
   int		  nontermcount;		/* Number of defined nonterminals */
   unsigned char *name;			/* Name for tables structure being generated */
   unsigned char *title;		/* Title of scanner and parser being generated */
   symbolentry	 *startsym;		/* Symbol entry for distinguished symbol */
   int		  repaircost;		/* Default error repair cost */
   int		  repaircontext;	/* Number of error repair context tokens */
   symbolentry	 *sentinel;		/* Symbol entry for end of input sentinel */
   treenode	 *scanner;		/* Regular expressions defined by input file */
   treenode	 *parser;		/* Grammar defined by input file */
   symbolentry	 *symboltable[HASH_TABLE_SIZE];	/* Hash table for defined symbols */

/* Data used to create the scanner being defined */

   dynarray	  nfapositions;		/* Sets of follow positions in NFA */
   dynarray	  dfastates;		/* Deterministic automaton state table */
   int		  dfacount;		/* Number of DFA states in scanner */

/* Data used to create the parser being defined */

   dynarray	  productions;		/* Grammar productions for parser */
   int		 *lhsindex;		/* Index of first production for each LHS in productions */
   dynarray	  collection;		/* Canonical collection of LR itemsets */
   firstset	 *first;		/* First sets for every token */
   int		 *errortoken;		/* Positive numbers are terminal tokens to be shifted */
   					/* Negative numbers are the negative production numbers to be reduced */
   int		**lrstates;		/* LR parsing table states */
   					/* Positive numbers greater than SHIFT_OFFSET are shift state numbers */
					/* Positive numbers less than SHIFT_OFFSET are shiftreduce production numbers */
					/* Negative numbers greater than ACCEPT_OFFSET are reduce production numbers */
					/* ACCEPT_OFFSET is the accept action */
};
